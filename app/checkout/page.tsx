...
import { 
  ArrowLeft, MapPin, ShoppingBag, Loader2, Navigation, ChevronRight
} from "lucide-react";
                        ...
                        <div className="space-y-3">
                                ...
                                <DialogTrigger asChild>
                                    <div 
                                        className={cn(
                                            ...
                                            !deliveryAddress ? "bg-amber-50 border-amber-200" : "bg-white"
                                        )}
                                    >
                                        <div className="flex items-center gap-3 overflow-hidden">
                                            ...
                                            <div className="flex flex-col truncate">
                                                <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">
                                                    {deliveryAddress ? "Selected Location" : "Action Required"}
                                                </span>
                                                <span className={cn("text-sm font-medium truncate", deliveryAddress ? "text-gray-900" : "text-amber-700")}>
                                                    {deliveryAddress || "Tap here to select delivery location on map"}
                                                </span>
                                            </div>
                                        </div>
                            ...
                            {!hasLocation && (
                                <p className="text-xs text-amber-600 pl-1">
                                    * We need your exact location to calculate delivery fees.
                                </p>
                            )}
                        </div>
                                ...
                                <div key={idx} className="flex gap-4 p-5">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex justify-between items-start">
                                            <h4 className="font-semibold text-gray-900 line-clamp-2">{item.menuItem.name}</h4>
                    ...
                    <CardContent className="p-6 space-y-6">
                        <div className="space-y-3 text-sm">
                            ...
                            <div className="flex justify-between">
                                ...
                                {isQuoting ? <Loader2 className="h-4 w-4 animate-spin text-[#7b1e3a]" /> : 
                                    <span className="font-semibold text-gray-900">{quote ? formatMoney(quote.deliveryFee) : "---"}</span>
                                }
                            </div>
                            <div className="flex justify-between">
                        ...
                        >
                            {isPlacingOrder ? <Loader2 className="animate-spin" /> : (!quote ? "Select Location to Continue" : "Pay Now")}
                        </Button>
                    </CardContent>
                </Card>
              </div>
