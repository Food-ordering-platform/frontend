...
import { LayoutDashboard, Users, WalletCards, LifeBuoy, LogOut, Menu } from "lucide-react";
import { Button } from "@/components/ui/button";
import { AdminProtectedRoute } from "@/components/auth/admin-protected-route";
import { useAuth } from "@/lib/auth-context";
import { Sheet, SheetContent, SheetTrigger, SheetHeader, SheetTitle } from "@/components/ui/sheet";

// 🟢 Notice the links stay clean without "/admin"
const navItems = [
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // 🟢 Next.js middleware exposes the internal rewritten path. We clean it up here for the UI logic.
  ...
  const SidebarContent = () => (
    <>
      <nav className="flex-1 p-4 space-y-1">
        {navItems.map((item) => {
          // 🟢 Check if the cleaned path matches our clean links
